...
def add():
    ...
    try:
        ...
        # sql语句
        sql = 'insert into receive(case_name, text, image, video) VALUES (%s,%s,%s,%s)'
        # %s对应的参数
...
def getData():
    ...
    try:
        ...
        if keyword.strip()=="":
            ...
            keyword = int(keyword)
            sql = 'select * from `receive` where id = %s'
            sql_params = [keyword, ]
        else:
            sql = 'select * from `receive` where %s'%column+" like %s"
            # %s对应的参数
            sql_params=['%'+keyword+'%',]
...
def edit():
    ...
    try:
        ...
        # sql语句
        sql = 'update `receive` set `case_name`=%s,text=%s,image=%s,video=%s where id = %s'
        # %s对应的参数
...
def delete():
    ...
    try:
        ...
        # sql语句
        for id in ids:
            sql = 'delete  from `receive` where id=%s'
            # %s对应的参数
            sql_params=[id,]
            cursor.execute(sql,sql_params)
