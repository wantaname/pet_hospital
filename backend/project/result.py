...
def add():
    ...
    try:
        ...
        # sql语句
        sql = 'insert into result(case_name, text, image, video) VALUES (%s,%s,%s,%s)'
        # %s对应的参数
...
def getData():
    ...
    try:
        ...
        if keyword.strip()=="":
            ...
            sql = 'select * from `result` where %s'%column+" like %s"
            # %s对应的参数
            sql_params=['%'+keyword+'%',]
...
def edit():
    ...
    try:
        ...
        # sql语句
        sql = 'update `result` set `case_name`=%s,text=%s,image=%s,video=%s where id = %s'
        # %s对应的参数
