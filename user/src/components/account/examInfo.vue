      ...
      <el-table
        ...
        <template v-for="(item,index) in table.table_columns">
          <el-table-column
              :prop="item.prop"
              :label="item.label"
              :width="item.width"  :key="index" v-if="item.prop!=='view'">
          </el-table-column>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button
                  ...
                  icon="el-icon-view" circle type="primary" plain title="查看">
              </el-button>
            </template>
          </el-table-column>
        </template>
...
export default {
  ...
  methods: {
    ...
    handleView(index,row){
      //
      this.$router.push({path:"/examDetail",query:{examId:row.id}})
    },
    //移除标签
    handleClose(tag) {
    ...
    //点击查看问题对话框
    handleQuestion(index, row) {
      //打开对话框
      this.editDialogVisible = true
      //根据id获得信息
      this.getQuestionById({column:"id",keyword:row.question_id})
