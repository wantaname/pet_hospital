  ...
  <div class="container">
    ...
    <el-card shadow="hover">
      ...
      <div slot="header">
        ...
        <el-input placeholder="请输入内容" v-model="keyword" class="search">
          <el-select v-model="column" slot="prepend" placeholder="请选择">
            <el-option :label="item.label" :value="item.value" v-for="(item,index) in search.options"
                       :key="index"></el-option>
      ...
      <el-table
        ...
        <!--        列-->
        <template v-for="(item,index) in table.table_columns">
          <el-table-column
              :prop="item.prop"
              :label="item.label"
              :width="item.width" :key="index" v-if="item.prop!=='question_id'">
          </el-table-column>
          <el-table-column
              :prop="item.prop"
              :label="item.label"
            ...
            <template slot-scope="scope">
              <el-button
                  size="small"
                  @click="handleQuestion(scope.$index, scope.row)"
                  circle type="success" plain>
    ...
    <el-dialog
      ...
      <div>
        ...
        <el-form :model="addInfo" label-width="100px">
          <el-form-item label="文件名" style="width:80%">
            <el-input v-model="addInfo.filename" type="text"></el-input>
          </el-form-item>
          <el-form-item label="路径" style="width:80%">
            <el-input v-model="'/static/image/'+addInfo.filename" type="text" disabled></el-input>
          </el-form-item>
          <el-form-item label="备注" style="width:80%">
            <el-input v-model="addInfo.note" type="text"></el-input>
          </el-form-item>
    ...
    <el-button type="primary" @click="editConfirm" disabled>确定</el-button>
  </span>
    </el-dialog>
...
//  根据id提交编辑
    ...
    clickAdd() {
      ...
      this.addInfo.filename = ""
      this.addInfo.note = ""

    },

    addConfirm() {
      if (this.addInfo.filename.indexOf(".") == -1) {
        return this.$message.error("文件名未指定类型！")
      }
      this.addFile()
    ...
    addFile() {
      ...
      this.$http
          .post(this.request.add.url, {
            name:this.addInfo.filename,
